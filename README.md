...
**AIとの対話中に現れる「検証行動（動的コンピテンシー）」を測り育てるB2B SaaSプラットフォームの、審査用プロトタイプ（B2B全体像モックUI ＋ 中核評価エンジン稼働）。**
...
### ① 組織分析・管理者ダッシュボード（Viability・モックUI）
* **対象**: 企業のDX推進責任者、技術部門長、人事・育成担当者
* **提供価値**: 組織全体のAI検証力・手戻りリスクを可視化し、研修投資対効果（ROI）を定量化
* **画面要素**:
  * **組織動的コンピテンシー4領域ヒートマップ**: 部署別（決済基盤、コアAPI、フロントエンド等）の検証力到達度
  * **AI盲従・過剰指摘リスク警告**: AI出力を無検証で承認している割合が高い部署の検知
  * **手戻り工数削減推移グラフ**: セッション蓄積に伴う実務手戻り削減率のシミュレーション推移
  * **他社・業界ベンチマーク比較**: 共通アンカー規準に基づく相対的位置づけ
