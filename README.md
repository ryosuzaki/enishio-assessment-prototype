...
**生成AIとの協働プロセスの中で立ち現れる「動的コンピテンシー（動的で不確実な状況下において、AIを含む情報やプロセスを多角的に吟味し、自己の思考を省察・適応させながら、責任を持って課題解決や価値創出を主導する能力）」を測り、育てるB2B SaaSのプロトタイプ。**（提案書では「AI時代の実務判断力を「共通の尺度」で測り育てる評価・育成基盤の開発」）
B2B全体像を示すモックUIと、中核評価エンジンの実稼働を1つのアプリケーションに統合している。
...
| 画面・機構 | 状態 |
| :--- | :--- |
| **実務演習セッション**（2ペイン動的対話 → 前提変化の注入 → CFF → 構造化採点パイプライン（2段階採点） → XAI評価レポート） | **LLMとDBを通って動作**（`src/lib/evaluator/`、`src/app/api/`） |
...
行数でいえばモック3画面とそのデータが約2,500行、評価エンジン（`src/lib` ＋ APIルート）が約2,800行、テストが約4,700行という比率になる。**モックが評価エンジンに匹敵する規模になるのは、1画面あたりの情報量が多い管理画面を3枚並べているためであり、動く部分を薄くした結果ではない。**
...
[課題提示・背景確認] ─→ [2ペイン動的対話] ─→ [CFF 事前判定] ─→ [構造化採点パイプライン（2段階）] ─→ [XAI評価レポート]
                              │ (場面3: ⚡ 緊急仕様変更の注入 ← 進行役が自動で撃つ)
```

| ステップ | 名称 | 受講者の体験 |
| :--- | :--- | :--- |
| **1** | **課題提示・背景確認** | 実務課題（決済セキュリティ／在庫並行処理／チケット自動割当）を選び、業務背景・3つの重点レビュー観点・スコープ外事項を確認して開始する。 |
| **2** | **2ペイン動的対話** | 左・中・右の3分割画面でAI同僚と協働レビューする。初期方針が固まった時点で進行役が緊急仕様変更（前提変化）を撃ち、適応力を観測する。**受講者はこの発火時点を選べない。** |
| **3** | **CFF（認知強制機能）** | AI採点の開示前に、成果物をGitHub PRレビュー準拠の3択（［承認］［条件付き承認］［修正要求］）で判定し、理由を確定させる。 |
| **4** | **構造化採点パイプライン（2段階採点）** | 対話ログから検証行動のスパンを抽出し（第1段階）、その証拠だけを入力に0〜5バンドで判定する（第2段階）。確信度が足りない判定は人間の確認待ちとして保留する。 |
...
**動的対話演習** — 上段左に業務要件・受入基準・運用コンテキスト（前提変化が起きるとここに緊急要件バナーが立つ）、上段右にAI同僚が書いた成果物コードとテストコード（コード選択から「チャットに引用」可能）。画面下部にAI同僚との対話と、進行役によるソクラテス型の深掘りが並び、右側サイドバーで状態推定をリアルタイムに観測できる。
...
* **セッション内の前提変化**
  レビューの途中で、クライアントからの緊急仕様変更を注入する仕掛け。当初方針に固執せず方針を更新できるか（メタ認知・適応力）を実測する。**発火は進行役側（`/api/dialogue/premise-shift`）が対話ログだけを見て決定論的に判定し、受講者は撃つ時点を選べない** `[D-100]`。受講者が撃てるなら身構えた状態での方針更新になり、撃たない自由がある限り証拠の被覆率が受講者ごとに変わって比較が成立しない。LLMにも委ねない——注入時点が生成側の裁量で揺れると、それ自体が比較不能な自由度になる。
...
| `VerificationFocusSequence` | コード検証順序（旧形式・非推奨）。現行仕様では対話ログ（`prompt_turns`）のコード引用ターンへ統合され、DBスキーマ互換性のため保持 |
...
`OPENAI_API_KEY` を `.env` へ設定すると、AI同僚との対話・構造化採点パイプラインによる採点・XAIレポートまで通る。未設定でも共通アンカー評価の出題とログ基盤は動く。
