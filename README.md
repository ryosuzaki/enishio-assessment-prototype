...
| 動画 | 内容 | 長さ |
| :--- | :--- | :--- |
| [実務演習セッション](https://github.com/user-attachments/assets/5185c4d1-fbb8-48e3-b77f-8463a809d4e0) | 課題提示 → 検証対話（コード引用・進行役の深掘り） → 前提変化 → 意思決定 → XAI診断・異議申立。最後にモック画面 | 約1分45秒 |
| [固定設問（SCT型）](https://github.com/user-attachments/assets/d85a75a7-54e8-4039-ae50-f26c3c451c9b) | 設問の選択 → 段階1〜3・3'（反論への応答） → 確信度 → 記録した回答の控え | 約1分10秒 |
...
| **固定設問（SCT型）** | **出題と回答記録まで実稼働** | 全受講者が同じ条件で解く、LLMを通さない外部基準。公開用サンプル3問を同梱。採点ロジックは実装済みで、専門家パネルの組成後に有効化（いまはダミー分布のため得点化しない） |
...
| 第1層 (2) LLM判定器による証拠の一対比較 | 事業期間中に実装（統計モデルの成立性は合成データで事前検証済み） |
| 第1層 (3) Bradley-Terry モデルによる共通尺度の推定 | 推定器は実装済み（[src/lib/equating/](src/lib/equating/)）。実データへの接続は事業期間中 |
| 第2層 (1) 固定設問（SCT型）の出題・記録・採点 | 出題と記録は実装済み。LLMを使わない採点ロジック（[src/lib/anchor-scoring.ts](src/lib/anchor-scoring.ts)）も実装済みで、専門家パネルの組成後に有効化（いまはダミー分布なので点を出さない） |
| 第2層 (2) 人間専門家の凍結ペア判定との突合 | 事業期間中に実装 |

> 💡 **共通尺度化の統計モデル検証について**  
> 「受講者ごとに課題が変わる動的アセスメントにおいて、本当に公平な比較（等化）が数理的に可能なのか」「LLM判定器の経時的ズレ（ドリフト）を検知できるのか」という技術的成立性については、実装に先立ち合成データシミュレーションで検証しています。詳細は **[共通尺度化エンジンの合成データ検証（docs/equating-simulation/）](docs/equating-simulation/README.md)** を参照してください（`npm run sim:equating` で手元でも再現可能です）。
...
**検証対話と前提変化**：左上に業務要件とコード、その下に計測ログと進行役の状態推定（計測ログは閉じられる）。右列はAI同僚との対話。
...
npm run test                  # 単体テスト（Vitest 266件・APIキー/DB不要）
...
* **[共通尺度化エンジンの合成データ検証](docs/equating-simulation/README.md)**: 課題難易度の交絡除去（Bradley-Terry）と判定器ドリフト検知に関する統計シミュレーション結果（図表・パラメータ一覧）
