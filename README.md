...
### ③ シナリオ別行動比較ギャラリー（Viability・モックUI ＆ 事後講評 Debriefing）
* **対象**: 受講者（演習後の復習・腹落ち）、EM・テックリード、人事育成責任者
* **提供価値**: 演習後に「専門家はどう振る舞ったのか」「他の受講者はどこで躓いたのか」を対比し、納得感の高い事後講評（Debriefing）と、社内での技術評価・AI操舵基準の合意形成を支援する。実運用のDBスキーマ（`PromptTurn`, `Rating`, `EvidenceComponent`, `RelianceMetrics`）と完全に整合する構造で設計。
* **画面要素**:
  * **シナリオセレクター & 受講者バンド分布**: 3つの実務課題における全受講者の評価分布（Level 0〜5）と蓄積セッション数の可視化
  * **現場の急所インサイト（Trap Insight）**: 「受講者の67%がユニットテストの正常系通過を見て異常系テストの欠落を見落とした」など、AI生成コードにありがちな典型的な落とし穴の根本原因を提示
  * **4つの代表的行動ペルソナ（比較対象）**:
    * 👑 **【模範】専門家（テックリード / Level 5）**: 正常系テストの罠を見抜き、PCI DSS規格やキャッシュフォールバック設計を提示してAI同僚に修正コードを再生成させる。
    * 💡 **【合格水準】中堅（Level 3〜4）**: 要件に基づいて不備を論理的に指摘しコード修正を完遂させるが、テストコード自体の抜け穴には言及せず。
    * ⚠️ **【典型的な罠】若手（Level 2）**: 違和感を持つが、AI同僚の「フェイルクローズ安全設計」という専門用語に丸め込まれて承認。
    * ❌ **【典型的な失敗】AI過信者（Level 1）**: ユニットテストが「All passed」であることを見て安心し、コードを読まずに即座に承認（blind acceptance）。
  * **詳細トレースビュー**: 事前判断の理由、実際の対話ログ（Dialogue Transcript）、コード差分（Before/After）、AutoSCORE 採点根拠 & エキスパート講評（意思決定の分岐点・教育的示唆）

![シナリオ別行動比較ギャラリー](docs/screenshots/09-benchmark-gallery.png)
...
> 
> ※ `OPENAI_API_KEY` が無い場合は実務課題対話〜採点（STEP 2〜4）は動作しません（代わりのローカル判定は置いていません）。なお、①組織ダッシュボード、②受講者カルテ、③行動比較ギャラリー、およびアンカー出題・ログ基盤はAPIキー不要で即座に閲覧・体験可能です。
