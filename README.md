...
> **どこが動いていて、どこがモックか。**上の 1 は**画面設計を示すためのモックUIであり、数値はすべてダミー値である**（該当画面にその旨を明示している）。実際にLLMとDBを通って動くのは 2 だけである。行数でいえばモック3画面＋そのデータが約2,800行、評価エンジン（`src/lib` ＋ API ルート）が約2,700行、テストが約4,600行という比率になる。**モックのほうが大きいのは、1画面あたりの情報量が多い管理画面を3枚並べているためであり、動く部分を薄くした結果ではない。**
