...
| ステップ | 名称 | 概要・受講者の体験 |
| :--- | :--- | :--- |
| **STEP 1** | **共通アンカー出題** | 共通アンカー項目へ回答。採点機の厳格度ドリフトを検知する固定基準点として無得点で客観記録（LLM採点を通さず循環遮断・初回必須＋以降7回に1回運用）。本プロトタイプでは出題〜無得点記録パイプラインを検証し、本番仕様では状況変化に対する判断更新を問う**疑似対話形式（SCT型）**へ発展。 |
...
> 
> ※ `ANTHROPIC_API_KEY` が無い場合は実務課題対話〜採点（STEP 2〜4）は動作しません（代わりのローカル判定は置いていません）。なお、①組織ダッシュボード、②受講者カルテ、およびアンカー出題・ログ基盤はAPIキー不要で即座に閲覧・体験可能です。
...
| **アンカー形式のSCT型への発展とプロトタイプの役割** | 状況変化に対する判断更新を追う「疑似対話形式（SCT型）」への発展を想定し、本プロトタイプでは出題から無得点記録までのパイプライン（Feasibility）を通す。疑似対話型バンクの本格構築とチート耐性実測は本格開発フェーズに行う |
...
`ANTHROPIC_API_KEY` を `.env` へ設定すると ②〜⑤（AI同僚との対話・AutoSCORE採点・XAIレポート）まで通る。未設定でも ① アンカー出題と ⓪ ログ基盤は動く。
...
**運用バンク20項目はこのリポジトリに含まれていない。**項目そのものを公開すると受検者が事前に読めてしまうためである（項目露出。MVP 2.6.2 の監視指標）。生成には隣の `enishio-education` リポジトリのチェックアウトが要る:
