...
* **提供価値**: 1回15〜30分の実務演習を通じて、AI協働プロセス（検証アプローチ・思考の癖・好手）を客観的エビデンスに基づき自己省察し、実務の設計・コードレビューですぐに使える判断軸と成長を実感する
* **画面要素**:
  ...
  * **協働アプローチ特性プロファイル（適正依存バランス）**: リスク防御力（CSR）と協働活用効率（CAR）のバランス、個人の検証スタイルの強みと実務での手戻りを防ぐ次の着眼点（※ネガティブなラベリングを廃止し、強みと成長機会として提示）
  * **演習ハイライト・好手（Good Moves）ログ**: AI同僚の反論を論理的に覆した発話や、前提変化に即応した設計判断など、優れた検証行動のエビデンス抽出
  * **実務転移チェックリスト（Takeaways）**: 演習から得られた「明日からのPRレビューで意識すべき自分専用の確認観点」
