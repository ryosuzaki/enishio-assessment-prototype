...
2. **中核評価エンジンの稼働（Feasibility・本物稼働）**: 1セッションが端から端まで通る縦切り（3ペイン動的対話・CFF・AutoSCORE 2段階採点・XAIレポート）を、Next.js + PostgreSQL + LLM API で実装し、APIキーを設定した環境で実際に稼働させて確認している。あわせてE2E 10件で画面遷移と分岐を自動検証している（**CIをAPIキー無しで回すため、E2EはAPIルートをモックしており、検証範囲は画面の結線と分岐である**）。
