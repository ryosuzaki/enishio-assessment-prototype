...
export function parseAnchorBankMarkdown(filePath: string): AnchorItemData[] {
  const content = fs.readFileSync(filePath, "utf-8");
  const lines = content.split("\n");

  const items: AnchorItemData[] = [];
  let currentItem: Partial<AnchorItemData> | null = null;
  let currentSection = "";
  let currentQ1Options: { key: string; text: string; note: string }[] = [];
  let currentQ2Options: { key: string; text: string; note: string }[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("### `ANCHOR-")) {
      if (currentItem && currentItem.anchor_id) {
        ...
        items.push(currentItem as AnchorItemData);
      }
...
// Execute parser and save to JSON
