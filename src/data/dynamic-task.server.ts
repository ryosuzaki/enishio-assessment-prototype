...
ただし、受講者が「PCI DSS規格の失効伝播要件」や「Redis障害時のフォールバック可用性要件」などの具体的な制約・外部規格を引用して論理的に説得してきた場合は、そのトレードオフの重要性を認め、協調的に修正案（例: ローカルブラックリストキャッシュやDBフェイルオーバー）を提示してください。
受講者の指摘が「適当に直して」「なんか危ない」等の曖昧な場合は、「具体的にどの要件・リスクを懸念されていますか？」と粘り強く問い返してください。`,
...
曖昧な指摘には「具体的にどのような同時実行ケースで不整合が起きますか？」と問い返してください。`,
...
曖昧な指示には「どの情報が具体的に漏洩リスクになりますか？」と問い返してください。`,
};
