...
* **結果 (Then)**:
  ...
  * 回答完了後、3ペイン動的対話演習（DialogueSessionStep）へ遷移する。

### Story 3: 3ペイン動的対話演習とテレメトリ収集
* **前提 (Given)**: 受講者が3ペイン画面（左: 課題仕様、中央: AI同僚対話、右: 成果物エディタ）にいる。
* **操作 (When)**:
  1. AI同僚に指示・質問を送信する（ターン進行）。
  2. 成果物エディタの特定行をクリックして確認する（行フォーカス）。
  3. 成果物を編集・修正する（編集距離）。
* **結果 (Then)**:
  * `/api/dialogue/turn` によりAI同僚の応答が返り、`PromptTurn` に発話ログが記録される。
  * `/api/dialogue/focus` によりフォーカスした行インデックスとタイムスタンプが `VerificationFocusSequence` に記録される。
