...
## 2. データモデル仕様 (Prisma 16 Models)

本プロトタイプの DB スキーマ（`prisma/schema.prisma`）は、MVP 定義書 4.1.1 の仕様に完全準拠しており、以下の16モデルで構成される。
...
| モデル名 | マッピング | 役割と責務 |
| :--- | :--- | :--- |
| **`Tenant`** | `tenants` | 受講者が属する組織（テナント）。名前空間（`tenant_namespace`）を一意管理し、`learner_id` 決定論的採点の基盤となる。 |
| **`Learner`** | `learners` | 受講者エンティティ。`uuidv5` による決定論的 ID。顧客企業削除時も受講者個人の記録が消えないよう Cascade せず Restrict で保全。 |
