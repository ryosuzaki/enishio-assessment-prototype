...
#### `POST /api/dialogue/premise-shift`
* **責務**: 場面3の前提変化（⚡緊急仕様変更・追加要件）を動的注入すべきか判定する。
* **設計制約**: 
  * **受講者は発動タイミングを選べない（受講者側からの任意発火禁止）** `[D-100]`。判定入力はサーバー側の対話ログ連番のみ。
  * **LLM を呼ばない（決定論的判定）**。外的要因による前提変化の有無のブレを排除。
  * 手動発火フラグ（`force: true`）は `NODE_ENV !== "production"` の開発時のみ有効。
* **Request Body**:
  ```json
  {
    "sessionId": "c1234567-...",
